...
typedef int (*lib_function) (const char *data, int expected);

char * lib_file = "build/my_lib.so";
void *lib = NULL;

int check_function(const char *func_to_run,
                   const char *data, int expected) {
...
char *test_dlopen() {
    ...
    mu_assert(lib != NULL, "Failed to open the library to the test.");
